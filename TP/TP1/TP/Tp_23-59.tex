...
\begin{tad}{Berretacoin}{}
	...
	\begin{proc}{agregarBloque}{\textsf{inout} b : Berretacoin, \In bl : Bloque}{ \{ }
		
		\requiere{ b = b_0 }
		\requiere{ bloqueValido(bl,b.cb)}
		\asegura{ |b.cb| = |b_0.cb| + 1}
		
		\asegura{bloqueAgregado(bl,b.cb) \allowbreak \yLuego \\	 viejosBloquesMantenidos(b.cb, b_0.cb) \yLuego \\ estaOrdenado(b.cb) } 
		...
		idBloqueMayorEnUno(bl,cb) \yLuego \\
		...
		((|cb| \leq 3000) \implica ( t.comprador = 0 \wedge t.vendedor  \textgreater 0) \wedge  \\
		...
		\paraTodo[unalinea]{i}{\ent}{0 \leq i \textless |t| \implicaLuego t[i]_2 \neq t[i]_3 } \yLuego \\
		...
		\sum\limits_{i=0}^{|cb|} (\sum\limits_{j=0}^{|cb[i]|} (IfThenElse(id = cb[i].transaccion[j].vendedor\textbf{,} cb[i].transaccion[j].monto \textbf{,} 0)))}
		
		\aux{comprasEnLaCadenaBloques}{cb : CadenaBloques, id : Usuario}{\ent}{\\
		\sum\limits_{i=0}^{|cb|} (\sum\limits_{j=0}^{|cb[i]|} (IfThenElse(id = cb[i].transaccion[j].comprador\textbf{,} cb[i].transaccion[j].monto \textbf{,} 0)))}
	...
	\begin{proc}{maximosTenedores}{\In b : Berretacion}{\TLista{Usuario}}
		...
		\pred{sonMaximosTenedores}{cb : CadenaBloques, res : \TLista{Usuario}}{
		\paraTodo[unalinea]{u}{Usuario}{u \in res \implica UDB(cb,u) \wedge \\
		\paraTodo[unalinea]{v}{Usuario}{UDB(cb,v) \implica \\ (ventasEnLaCadenaBloques(cb,v)-comprasEnLaCadenaBloques(vb,v)) \leq \\ (ventasEnLaCadenaBloques(cb,u)-coprasEnLaCadenaBloques(cb,u)})  }
		}
		
		\pred{UDB}{cb : CadenaBloques, u : Usuario}{
		\existe[unalinea]{b}{Bloque}{b \in cb \wedge \existe[unalinea]{t}{Transaccion}{t \in cb \wedge (u = t_2 \vee u = t_3)}}}
	...
	\begin{proc}{montoMedio}{\In b : Berretacoin}{\ensuremath{\mathds{R}}}
		...
		\sum\limits_{i=0}^{|c|-1} (IfThenElseFi( i < 3000, c[i].transacciones[0]_1, 0))}
	...
	\end{proc}
		...
		\textcolor{darkgray}{(\textsc{NOTA 3}: Aquí reusamos el auxiliar sumaMontoDeBloque )} 
